...
# ---- TEACHER-STUDENT MODEL (pure numpy — no torch required) ----
TS_READY = False
_weights = None
_ts_le   = None
...
try:
    ...
    _ts_le    = joblib.load(os.path.join(MODEL_DIR, "ts_labels.pkl"))
    _weights  = dict(np.load(os.path.join(MODEL_DIR, "student_weights_numpy.npz")))
    ...
    def _batchnorm(x, w):
        eps = 1e-5
        return (w['encoder.0.weight'] *
                (x - w['encoder.0.running_mean']) /
                np.sqrt(w['encoder.0.running_var'] + eps) +
                w['encoder.0.bias'])

    def _infer(avg_values):
        """Run student MLP forward pass entirely in numpy."""
        x = np.array(avg_values, dtype=np.float32)
        x = _batchnorm(x, _weights)
        x = _relu(x @ _weights['encoder.1.weight'].T + _weights['encoder.1.bias'])
        x = _relu(x @ _weights['encoder.3.weight'].T + _weights['encoder.3.bias'])
        x = x @ _weights['classifier.weight'].T + _weights['classifier.bias']
        return _softmax(x)

    TS_READY = True
    print(f"Student model loaded (numpy). Classes: {list(_ts_le.classes_)}")

except Exception as e:
    print(f"Student model not loaded ({e}) — classification disabled")
...
# Material label
rec_label = QtWidgets.QLabel("Material:")
...
label_input.setFixedWidth(220)
rec_hl.addWidget(label_input)

# Height dropdown — mandatory
height_lbl = QtWidgets.QLabel("Height:")
height_lbl.setStyleSheet("font-size: 13px; font-weight: bold;")
rec_hl.addWidget(height_lbl)

height_combo = QtWidgets.QComboBox()
height_combo.addItem("-- select --")
for h in ["0cm", "5cm", "10cm", "15cm", "20cm"]:
    height_combo.addItem(h)
height_combo.setFixedWidth(110)
height_combo.setStyleSheet("""
    QComboBox {
...
pred_frame.setStyleSheet("""
    ...
    QLabel { border: none; }
""")
pred_frame.setMaximumHeight(60)
pred_hl = QtWidgets.QHBoxLayout(pred_frame)
pred_hl.setContentsMargins(14, 6, 14, 6)
pred_hl.setSpacing(14)

pred_model_lbl = QtWidgets.QLabel("🧠 Student Model:")
pred_model_lbl.setStyleSheet("font-size: 13px; font-weight: bold; color: #aaaaaa;")
pred_hl.addWidget(pred_model_lbl)

pred_class_lbl = QtWidgets.QLabel("—" if not TS_READY else "Waiting...")
pred_class_lbl.setStyleSheet("font-size: 20px; font-weight: bold; color: #ffffff; min-width: 160px;")
pred_hl.addWidget(pred_class_lbl)

pred_conf_lbl = QtWidgets.QLabel("")
pred_conf_lbl.setStyleSheet("font-size: 11px; color: #888888;")
pred_hl.addWidget(pred_conf_lbl)

pred_hl.addStretch()

if not TS_READY:
    no_model_lbl = QtWidgets.QLabel("⚠  student_weights.pth / ts_labels.pkl not found — run teacher_student_pipeline.py first")
    no_model_lbl.setStyleSheet("font-size: 11px; color: #ffaa00;")
    pred_hl.addWidget(no_model_lbl)

main_vl.addWidget(pred_frame)
...
def start_recording():
    ...
    height = height_combo.currentText()
    ...
    if height == "-- select --":
        status_lbl.setText("⚠  Select a sensor height first!")
        status_lbl.setStyleSheet("font-size: 13px; color: #ffaa00; padding: 4px 12px;")
        return

    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = os.path.join(SAVE_DIR, f"{label}_{height}_{timestamp}.csv")

    # Build CSV header — includes height column
    header = ["label", "height_cm", "timestamp"]
    ...
    status_lbl.setText(f"⏺  Recording: {label} @ {height}{limit_str}")
...
def update():
    ...
    if TS_READY:
        try:
            ...
            pred_name = _ts_le.inverse_transform([pred_idx])[0]
            ...
            pred_class_lbl.setText(pred_name)
            pred_class_lbl.setStyleSheet(
                f"font-size: 20px; font-weight: bold; color: {color}; min-width: 160px;")

            class_probs = sorted(zip(_ts_le.classes_, probs), key=lambda x: -x[1])
            prob_str = "   ".join(f"{c}: {p*100:.0f}%" for c, p in class_probs)
            pred_conf_lbl.setText(prob_str)
        except Exception:
            pass
